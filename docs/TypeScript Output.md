...
Generated theme assets are separate:
...
Navigation, search, build information, and copied content assets are planned extensions; they are not emitted by the current schema-1 generator. Each generated TypeScript content module begins with a clear “Generated by MamboSite; do not edit” comment. The manifest, page records, generated theme metadata, and ownership markers record their schema version; the aggregate `pages/index.ts` module does not repeat it. Output paths must be stable across operating systems.
...
- When content-asset generation is added, asset IDs will derive from content hashes so identical assets may be deduplicated.
...
Nodes contain only serializable authored data, child nodes, and optional source spans. Resolved links and embeds sit beside the tree. Generated records never contain functions, React elements, class names, or executable source.
...
Resolved embeds remain explicit graph edges while their authored AST nodes stay in the Markdown tree:
...
Compiler or theme-validation errors publish nothing. TypeScript and theme assets are separate managed trees, so their replacement is not one cross-directory transaction.
